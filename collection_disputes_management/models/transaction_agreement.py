...
class TransactionAgreement(models.Model):
    _name = 'collection_disputes_management.transaction_agreement'
    ...
    interest_due = fields.Monetary(related='settlement_statement_id.interest_due', string="Intérêts", readonly=True)
    commitment_fees = fields.Monetary(related='settlement_statement_id.commitment_fees', string="Commissions d'engagement", readonly=True)
    late_interest = fields.Monetary(related='settlement_statement_id.late_interest', string="Intérêts de retard", readonly=True)
    agf_fees = fields.Monetary(related='settlement_statement_id.agf_fees', string="Commissions AGF", readonly=True)
    ...
    interest = fields.Monetary(string="Intérêts", tracking=True)
    interest_rate = fields.Float(string=u"Taux d'intérêt", tracking=True)
    late_interest_rate = fields.Monetary(string=u"Taux d'intérêt de retard", tracking=True)
    commitment_fees_agreement = fields.Monetary(string="Commissions d'engagement")
    late_interest_agreement = fields.Monetary(string="Intérêts de retard")
    agf_fees_agreement = fields.Monetary(string="Commissions AGF")
    ...
    negotiation_round_ids = fields.One2many('collection_disputes_management.negotiation_round', 'agreement_id', string="Rounds de négociation")
    amortization_line_ids = fields.One2many('collection_disputes_management.amortization_line','agreement_id', string="Echéances / Paiements")
    ...
    def _compute_annuity(self, P, i, number_of_installments, amortization_type='fixed_amount'):
        ...
        return P * i / (1 - (1 + i) ** (-number_of_installments))
    ...
    def action_generate_amortization(self):
        ...
        for rec in self:
            ...
            annual_rate = (rec.interest_rate or 0.0) / 100.0
            ...
            M = rec._compute_annuity(effective_principal, i, n)
            ...
            rec.env['collection_disputes_management.amortization_line'].create(lines_vals)