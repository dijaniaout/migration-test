...
class CollectionDisputeCase(models.Model):
    ...
    customer_id = fields.Many2one('collection_disputes_management.customer', string='Débiteur', required=True)
    guarantor_ids = fields.Many2many('collection_disputes_management.guarantor', 'guarantor_rec', string='Garants', required=True)
    collateral_ids = fields.One2many('collection_disputes_management.collateral', 'dispute_case_id', string="Garanties")
    customer_activity_sector_id = fields.Many2one('collection_disputes_management.business_sector', string=u"Secteur d'activité du client", related='customer_id.activity_sector_id', store=True)
    customer_legal_status_id = fields.Many2one('collection_disputes_management.legal_status', string="Forme juridique du client", related='customer_id.legal_status_id', store=True)
    ...
    tag_ids = fields.Many2many('collection_disputes_management.tag', string="Étiquettes")
    ...
    legal_dispute_ids = fields.One2many('collection_disputes_management.legal_dispute','collection_dispute_case_id',string="Litiges")
    justice_intervention_ids = fields.One2many('collection_disputes_management.justice_intervention', 'dispute_case_id', string="Interventions de Justice")
    additional_fee_ids = fields.One2many('collection_disputes_management.additional_fee', 'dispute_case_id', string="Frais annexes")
    ...
    justice_auxiliary_ids = fields.Many2many("collection_disputes_management.justice_auxiliary", "justice_auxiliary_rec", string="Conseiller juridique")
    ...
    loan_duration = fields.Integer(string="Durée du prêt (mois)")
    transaction_agreement_ids = fields.One2many('collection_disputes_management.transaction_agreement','dispute_case_id',string="Accords Transactionnels")
    # Forward of amortisation lines from the validated aggreement
    amortization_line_ids = fields.One2many('collection_disputes_management.amortization_line','case_id', string="Echéances / Paiements")
    ...
    def _default_base_workflow_id(self):
        return self.env.ref('collection_disputes_management.workflow_recovery', raise_if_not_found=False)
    ...
    def action_open_amortization_lines(self):
        return {
            ...
            'res_model': 'collection_disputes_management.amortization_line',
            'view_mode': 'list,form',
    ...
    def action_open_legal_dispute(self):
        return {
            'name': 'Litiges',
            'type': 'ir.actions.act_window',
            'res_model': 'collection_disputes_management.legal_dispute',
            'view_mode': 'list,form',
